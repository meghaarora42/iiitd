...
// Q.1 LBP
// Implement LBP, use 3x3 square neighborhood, divide face image into non overlapping 8x8 patches of equal 
// sizes. Show LBP pattern image example. Use non weighted chi square distance measure to obtain match scores.

// Experiment protocol: Use first four images of every person as the enrollment templates, and remaining 
// as query samples. When comparing a query image against all the enrollment images, the one which has the 
// highest match-scores (least distance score) with query is declared the predicted match. And, the corresponding
